...
import { AI_PROVIDER_CATALOG, getProviderSettings, saveProviderSettings } from "./provider-config.js";
...
agenticProviderRoutes.get("/provider-catalog", requireScope("school:read"), c => c.json({
  data: {
    ...
    limits: {
      temperature: { min: 0, max: 2 }, topP: { min: 0, max: 1 },
      maxOutputTokens: { min: 128, max: 65536 }, timeoutMs: { min: 5000, max: 120000 },
...
agenticProviderRoutes.get("/settings", requireScope("school:read"), async c => {
  ...
  return c.json({ data: {
    provider: catalog.label, providerId: settings.provider, configured: settings.configured,
    apiKeyConfigured: settings.apiKeyConfigured, apiKeyHint: settings.apiKeyHint,
    models: settings.models, config: settings.config, source: settings.source, updatedAt: settings.updatedAt,
  }});
});
...
agenticProviderRoutes.patch("/provider-settings", requireScope("school:write"), async c => {
  ...
  const input = { ...parsed.data, clearApiKey: Boolean(parsed.data.clearApiKey || (providerChanged && !hasReplacementKey)) };
