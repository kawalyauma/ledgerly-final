...
agenticLightRoutes.post("/chat-studio/conversations/:id/guided-analysis",requireScope("school:read"),async c=>{
 ...
 await c.env.FINANCE_DB.batch([
  c.env.FINANCE_DB.prepare("INSERT INTO ae_messages(id,organization_id,conversation_id,role,content,user_id,model,metadata_json) VALUES(?,?,?,'assistant',?,?,?,?)").bind(assistantId,p.organizationId,thread.id,summary.slice(0,12000),p.userId,result.model||"guided-analysis",JSON.stringify({mode,topic:result.topic?.id,entity:result.entity,plan:result.plan,sources:result.sources,usage:result.usage})),
