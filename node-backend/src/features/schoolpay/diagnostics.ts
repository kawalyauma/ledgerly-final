...
type EventStats = { failed: number; unmatched: number; received: number; posted24h: number; latestEventAt: string | null };
type AdhocStats = { pending: number; postingFailed: number; failed: number; paid24h: number; recoveryDue: number; stalePending: number; oldestPendingAt: string | null; recoveryErrors24h: number };
type ReconciliationRow = { id: string; status: string; fromDate: string; toDate: string; startedAt: string; completedAt: string | null; error: string | null };
type SecurityStats = { processed24h: number; rejectedIp24h: number; invalidPayload24h: number; failed24h: number; latestAuditAt: string | null };
...
export async function getSchoolPayDiagnostics(runtime: Runtime, organizationId: string) {
  ...
  if (!config.configured) {
    return { status: "not_configured" as const, configured: false, recommendations: ["Configure this school's SchoolPay account before accepting payments."] };
  }

  const [events, adhoc, reconciliation, security] = await Promise.all([
    runtime.db.query<EventStats>(
      `SELECT COUNT(*) FILTER (WHERE status='failed')::int AS failed,
       ...
       FROM schoolpay_events WHERE organization_id=$1`, [organizationId]),
    runtime.db.query<AdhocStats>(
      `SELECT COUNT(*) FILTER (WHERE status='pending')::int AS pending,
       ...
       FROM schoolpay_adhoc_intents WHERE organization_id=$1`, [organizationId]),
    runtime.db.query<ReconciliationRow>(
      `SELECT id,status,from_date::text AS "fromDate",to_date::text AS "toDate",started_at::text AS "startedAt",
       ...
       FROM schoolpay_reconciliation_runs WHERE organization_id=$1 ORDER BY started_at DESC LIMIT 1`, [organizationId]),
    runtime.db.query<SecurityStats>(
      `SELECT COUNT(*) FILTER (WHERE outcome='processed' AND created_at>=CURRENT_TIMESTAMP-INTERVAL '24 hours')::int AS "processed24h",
         COUNT(*) FILTER (WHERE outcome='rejected_ip' AND created_at>=CURRENT_TIMESTAMP-INTERVAL '24 hours')::int AS "rejectedIp24h",
         COUNT(*) FILTER (WHERE outcome='invalid_payload' AND created_at>=CURRENT_TIMESTAMP-INTERVAL '24 hours')::int AS "invalidPayload24h",
         COUNT(*) FILTER (WHERE outcome='failed' AND created_at>=CURRENT_TIMESTAMP-INTERVAL '24 hours')::int AS "failed24h",
         MAX(created_at)::text AS "latestAuditAt"
       FROM schoolpay_security_audit WHERE organization_id=$1`, [organizationId]),
  ]);

  const eventStats = events.rows[0] ?? { failed: 0, unmatched: 0, received: 0, posted24h: 0, latestEventAt: null };
  const adhocStats = adhoc.rows[0] ?? { pending: 0, postingFailed: 0, failed: 0, paid24h: 0, recoveryDue: 0, stalePending: 0, oldestPendingAt: null, recoveryErrors24h: 0 };
  const securityStats = security.rows[0] ?? { processed24h: 0, rejectedIp24h: 0, invalidPayload24h: 0, failed24h: 0, latestAuditAt: null };
  ...
  const allowlistConfigured = runtime.config.SCHOOLPAY_WEBHOOK_IP_ALLOWLIST.length > 0;
  const allowlistEnforced = runtime.config.SCHOOLPAY_ENFORCE_WEBHOOK_IP_ALLOWLIST;
  ...
  if (config.enabled) {
    ...
    } else if (reconciliationAgeMinutes === null || reconciliationAgeMinutes > 180 || adhocStats.recoveryDue > 0
      || adhocStats.recoveryErrors24h > 0 || !runtime.config.SCHOOLPAY_PUBLIC_BASE_URL || !allowlistEnforced || securityStats.failed24h > 0) {
      status = "degraded";
    }
  }
  ...
  if (!allowlistConfigured) recommendations.push("Request SchoolPay's official webhook source IPs and configure SCHOOLPAY_WEBHOOK_IP_ALLOWLIST.");
  else if (!allowlistEnforced) recommendations.push("After verifying the SchoolPay IP list in production, set SCHOOLPAY_ENFORCE_WEBHOOK_IP_ALLOWLIST=true.");
  ...
  if (securityStats.rejectedIp24h > 0) recommendations.push("Review rejected SchoolPay source IPs in the security audit before changing the allowlist.");
  if (securityStats.failed24h > 0) recommendations.push("Review recent failed webhook processing entries in the SchoolPay security audit.");
  ...
  return {
    ...
    security: {
      ...securityStats,
      ipAllowlistConfigured: allowlistConfigured,
      ipAllowlistEnforced: allowlistEnforced,
      allowlistRuleCount: runtime.config.SCHOOLPAY_WEBHOOK_IP_ALLOWLIST.length,
      trustedProxyRuleCount: runtime.config.SCHOOLPAY_TRUSTED_PROXY_IPS.length,
    },
    latestReconciliation,
    schedules: { transactionReconciliation: "hourly at minute 15", adhocRecovery: "every 5 minutes" },
    recommendations,
  };
}
