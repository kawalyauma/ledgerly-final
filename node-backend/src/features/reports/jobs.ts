...
export async function handleReportExport(job: ClaimedJob, runtime: Runtime): Promise<void> {
  ...
  await runtime.db.query(
    `UPDATE report_jobs SET status='running',started_at=COALESCE(started_at,CURRENT_TIMESTAMP),error=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=$1`,
    [record.id],
  );
  try {
    ...
    await runtime.db.query(
      `UPDATE report_jobs SET status='completed',object_key=$1,content_type=$2,size_bytes=$3,error=NULL,completed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=$4`,
      [key, reportContentTypes[record.format], body.byteLength, record.id],
    );
  } catch (error) {
    await runtime.db.query(
      `UPDATE report_jobs SET status='failed',error=$1,updated_at=CURRENT_TIMESTAMP WHERE id=$2`,
      [error instanceof Error ? error.message : String(error), record.id],
    );
    throw error;
  }
}
