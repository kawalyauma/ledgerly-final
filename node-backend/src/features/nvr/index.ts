...
export const nvrFeature: BackendFeature={
  key:'nvr',
  version:'1.5.0',
  mount(app,runtime){app.route('/api/v1/nvr',createNvrRoutes(runtime));app.route('/api/v1/nvr',createNvrPairingRoutes(runtime));app.route('/api/v1/nvr',createNvrDeviceRoutes(runtime));app.route('/api/v1/nvr',createNvrRecordingDeviceRoutes(runtime));app.route('/api/v1/nvr',createNvrRetentionRoutes(runtime));app.route('/api/v1/nvr',createNvrPlaybackRoutes(runtime));app.route('/api/v1/nvr',createNvrViewerSignalRoutes(runtime));app.route('/api/v1/nvr',createNvrDeviceSignalRoutes(runtime));},
  registerJobs(registry){registry.register('nvr.retention.sweep',sweepNvrRetention);registry.register('nvr.offline.sweep',sweepNvrOffline);},
  schedules:[{name:'nvr-retention-sweep',cron:'23 * * * *',kind:'nvr.retention.sweep',queue:'nvr',maxAttempts:3},{name:'nvr-offline-sweep',cron:'* * * * *',kind:'nvr.offline.sweep',queue:'nvr',maxAttempts:3}],
};
