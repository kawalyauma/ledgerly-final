...
export async function syncCurriculumLoadRules(db:D1Database,org:string,userId:string,timetableId:string){
 ...
 const statements=loads.map(x=>db.prepare(`INSERT INTO acad_timetable_rules(id,organization_id,timetable_id,scope_type,scope_id,rule_type,config_json,hardness,priority,active,notes,created_by) VALUES(?,?,?,'subject',?,'weekly_periods',?,'soft',90,1,?,?)`).bind(createId("atr"),org,timetableId,x.subjectId,JSON.stringify({classId:x.classId,periods:Number(x.periodsPerWeek),source:"school_class_subjects"}),`[canonical-load] ${x.subjectName}: ${x.periodsPerWeek} period(s) per week`,userId));
