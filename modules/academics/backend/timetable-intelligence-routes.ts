...
const ACTIVE_RULE_TYPES=new Set(["weekly_periods","teacher_daily_max","teacher_consecutive_max","class_daily_subject_max","preferred_period","avoid_period","require_double","subject_spread","subject_min_gap","morning_preference","fixed_slot","working_days"]);
...
function normalizeRule(d:any){const type=String(d.ruleType||""),cfg=d.config&&typeof d.config==="object"?d.config:{};if(type==="fixed_slot"){if(String(d.scopeType||"")!=="subject")throw new AppError(422,"FIXED_SLOT_SCOPE","A fixed slot must target a subject; choose the class and optional stream in the fixed-slot details.");if(!String(cfg.classId||"").trim())throw new AppError(422,"FIXED_SLOT_CLASS","Fixed slot class is required.");if(!Number(cfg.day)||Number(cfg.day)<1||Number(cfg.day)>7)throw new AppError(422,"FIXED_SLOT_DAY","Fixed slot day is required.");if(!String(cfg.periodId||"").trim())throw new AppError(422,"FIXED_SLOT_PERIOD","Fixed slot period is required.");if(cfg.lessonCount!=null&&!new Set([1,2]).has(Number(cfg.lessonCount)))throw new AppError(422,"FIXED_SLOT_LENGTH","Fixed slot length must be one or two periods.");d.hardness="hard";d.priority=100;}if(type==="working_days"){const days=Array.isArray(cfg.days)?[...new Set(cfg.days.map(Number).filter((n:number)=>n>=1&&n<=7))]:[];if(!days.length)throw new AppError(422,"WORKING_DAYS_REQUIRED","Choose at least one working day.");d.scopeType="school";d.scopeId=null;d.config={...cfg,days};d.hardness="hard";d.priority=100;}return d;}
...
academicsTimetableIntelligenceRoutes.post("/timetables/:id/rules",...write,async c=>{const p=principal(c),d=normalizeRule(await body(c));validateScopedInput(d,"Rule");if(!ACTIVE_RULE_TYPES.has(String(d.ruleType||"")))throw new AppError(422,"RULE_NOT_ACTIVE",`Rule ${String(d.ruleType||"")} is reserved but is not active in the professional drafting engine yet.`);return c.json({data:await T.createRule(c.env.FINANCE_DB,p.organizationId,p.userId,c.req.param("id"),d)},201);});
