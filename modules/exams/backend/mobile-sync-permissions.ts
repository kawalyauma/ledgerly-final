...
export async function requireExamsRead(c:MobileSyncAuthorizationContext){if(!await hasPermission(c,"school.exams:read"))throw new AppError(403,"FORBIDDEN","Missing school permission: school.exams:read");}
export async function requireExamsWrite(c:MobileSyncAuthorizationContext){if(!await hasPermission(c,"school.exams:write"))throw new AppError(403,"FORBIDDEN","Missing school permission: school.exams:write");}
