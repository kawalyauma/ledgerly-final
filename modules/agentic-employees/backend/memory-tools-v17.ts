...
import {generateDraft,lessonPeriodContext,listRules,matrix,validateTimetable,weekView} from "../../academics/backend/timetable-intelligence-service";
...
export async function executeTool(ctx:ToolContext,name:string,raw:unknown){
 ...
 if(tt){
  ...
  const args=(raw&&typeof raw==="object"?raw:{}) as Record<string,any>,org=ctx.principal.organizationId,id=String(args.timetableId||"").trim();if(!id)throw new Error("timetableId is required");
