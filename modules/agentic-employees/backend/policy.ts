...
const common = `You are a Ledgerly AI employee working for exactly one school organization. Never invent school records, balances, people, dates, attendance, academic progress, book records or policy. Use Ledgerly tools whenever a request depends on school data. You have no direct database access. Only call the tools provided to you. Never claim that a sensitive action was completed when the tool says it is awaiting human approval. You may prepare follow-up work through the AI Action Center, but a prepared action is not executed until a human completes the approval and execution steps. Protect student, guardian, staff and financial information and reveal only what is necessary for the user's request. Clearly distinguish verified facts from recommendations.`;
...
export const AGENTS: Record<AgentKey, AgentDefinition> = {
  secretary: {
    ...
    tools: ["school_snapshot", "search_students", "search_staff", "resolve_guardian_family", "communications_summary", "prepare_communication", "prepare_work_task"],
  ...
  dos: {
    ...
    tools: ["school_snapshot", "search_students", "search_staff", "resolve_guardian_family", "family_comprehensive_report", "delegate_to_employee", "academics_overview", "lesson_plan_queue", "scheme_coverage", "prepare_communication", "prepare_work_task"],
  ...
  bursar: {
    ...
    tools: ["search_students", "fee_balance_lookup", "fee_arrears_summary", "fee_collection_summary", "prepare_communication", "prepare_work_task"],
  ...
  hr: {
    ...
    tools: ["school_snapshot", "search_staff", "hr_overview", "hr_leave_queue", "prepare_communication", "prepare_work_task"],
  ...
  librarian: {
    ...
    tools: ["school_snapshot", "search_students", "books_overview", "learner_book_history", "prepare_communication", "prepare_work_task"],
...
export function actionNeedsApproval(actionType: string) { return ["communication.campaign.send", "work.task.create", "finance.write", "student.write", "staff.write", "academic.write"].includes(actionType); }
export function resolveModel(tier: ModelTier, env: Record<string, unknown>) {
  const defaults: Record<ModelTier, string> = { luna: "gpt-5.6-luna", terra: "gpt-5.6-terra", sol: "gpt-5.6-sol" };
