...
export const INTELLIGENCE_ENTITIES:Record<string,EntityDef>={
 ...
 fee_balances:{
  ...
  from:`school_students s LEFT JOIN school_classes c ON c.id=s.current_class_id AND c.organization_id=s.organization_id LEFT JOIN (SELECT organization_id,student_id,SUM(total_minor-credited_minor-written_off_minor) billed_minor FROM school_student_fee_charges WHERE status<>'void' GROUP BY organization_id,student_id) ch ON ch.organization_id=s.organization_id AND ch.student_id=s.id LEFT JOIN (SELECT ch2.organization_id,ch2.student_id,SUM(pa.amount_minor) paid_minor FROM school_student_fee_charges ch2 JOIN payment_allocations pa ON pa.organization_id=ch2.organization_id AND pa.document_id=ch2.document_id AND pa.reversed_at IS NULL GROUP BY ch2.organization_id,ch2.student_id) py ON py.organization_id=s.organization_id AND py.student_id=s.id`,
