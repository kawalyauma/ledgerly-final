...
export async function executeQuickCommand(input:{db:D1Database;env:Env;principal:AuthPrincipal;agent:AgentDefinition;conversationId:string;registry:LightToolRegistry;toolName:string;values:Record<string,unknown>;commandText?:string}){
  ...
  if(tool.source==="native"){
    ...
    if(tool.readOnly){const response=await input.env.AGENT_SYSTEM_GATEWAY.request({agentKey:input.agent.key,principal:input.principal,method:tool.method||"GET",path:filled.path,body:tool.method==="GET"?undefined:resolved.value});if(!response.ok){const status=response.status===400||response.status===401||response.status===403||response.status===404||response.status===409||response.status===422||response.status===500||response.status===503?response.status:500;const data=response.data as any;throw new AppError(status,"COMMAND_FAILED",data?.error?.message||`Ledgerly returned HTTP ${response.status}`);}result=response.data;}
