...
agenticLightRoutes.post("/chat-studio/conversations/:id/guided-analysis",requireScope("school:read"),async c=>{
 ...
 await c.env.FINANCE_DB.batch([
  c.env.FINANCE_DB.prepare("INSERT INTO ae_messages(id,organization_id,conversation_id,role,content,user_id,model,metadata_json) VALUES(?,?,?,'assistant',?,?,?,?)").bind(assistantId,p.organizationId,thread.id,summary.slice(0,12000),p.userId,result.model||"guided-analysis",JSON.stringify({mode,topic:result.topic?.id,entity:result.entity,plan:result.plan,sources:result.sources,responseMeta:(result as any).responseMeta})),
...
agenticLightRoutes.post("/chat-studio/conversations/:id/composite-report",requireScope("school:read"),async c=>{
 ...
 const assistantMessageId=createId("aam"),summary=result.needsCriteria?"Composite report needs criteria: "+result.questions.join(" "):String((result as any).humanResponse||("Composite report completed with "+result.data.rows.length+" row(s)."));
 await c.env.FINANCE_DB.batch([
  c.env.FINANCE_DB.prepare("INSERT INTO ae_messages(id,organization_id,conversation_id,role,content,user_id,model,metadata_json) VALUES(?,?,?,'assistant',?,?,?,?)").bind(assistantMessageId,principal.organizationId,thread.id,summary.slice(0,12000),principal.userId,result.model||"composite-report",JSON.stringify({mode:"composite-report",plan:result.plan,sources:result.sources,responseMeta:(result as any).responseMeta})),
