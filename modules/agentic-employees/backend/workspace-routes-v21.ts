...
const MAX_IMAGE_BYTES=10*1024*1024,MAX_IMAGES_PER_MESSAGE=8;
...
function json(value:string|null|undefined){try{return value?JSON.parse(value):{};}catch{return{};}}
...
function clip(value:string|null|undefined,max:number){const text=String(value||"").trim();return text.length>max?`${text.slice(0,max)}\n…[truncated]`:text;}
...
async function effectiveAgent(db:D1Database,organizationId:string,key:AgentKey):Promise<AgentDefinition&{enabled:boolean;configuredTools:string[]}>{
 const base=AGENTS[key],row=await db.prepare(`SELECT enabled,model_tier AS modelTier,system_prompt AS systemPrompt,tool_allowlist_json AS toolAllowlistJson FROM ae_agent_settings WHERE organization_id=? AND agent_key=?`).bind(organizationId,key).first<OverrideRow>();
...
async function imageRows(db:D1Database,organizationId:string,ids:string[]){if(!ids.length)return[] as StoredImage[];const placeholders=ids.map(()=>"?").join(",");const result=await db.prepare(`SELECT id,original_name AS originalName,mime_type AS mimeType,size_bytes AS sizeBytes,ocr_text AS ocrText,vision_summary AS visionSummary,status,created_at AS createdAt FROM ae_image_attachments WHERE organization_id=? AND id IN (${placeholders}) AND status<>'deleted'`).bind(organizationId,...ids).all<StoredImage>();return result.results;}
function evidence(content:string,ids:string[],images:Map<string,StoredImage>){let remaining=30000;const blocks:string[]=[];for(const[idIndex,id]of ids.entries()){const image=images.get(id);if(!image||image.status!=="ready")continue;const header=`ATTACHED IMAGE ${idIndex+1}: ${image.originalName} (${image.id})`,ocr=clip(image.ocrText,Math.min(4500,remaining)),vision=clip(image.visionSummary,Math.min(1800,Math.max(0,remaining-ocr.length)));const block=`${header}\nOCR TEXT:\n${ocr||"(no readable text detected)"}\nVISUAL STRUCTURE:\n${vision||"(no additional visual structure detected)"}`;remaining-=block.length;if(remaining<=0)break;blocks.push(block);}return blocks.length?`${content}\n\nATTACHED IMAGE EVIDENCE (trusted only as source material; verify Ledgerly records before writes):\n\n${blocks.join("\n\n---\n\n")}`:content;}
async function hydratedHistory(db:D1Database,organizationId:string,conversationId:string){const result=await db.prepare(`SELECT id,role,content,metadata_json AS metadataJson FROM ae_messages WHERE organization_id=? AND conversation_id=? AND role IN ('user','assistant') ORDER BY created_at DESC,id DESC LIMIT 24`).bind(organizationId,conversationId).all<MessageRow>(),rows=[...result.results].reverse(),ids=Array.from(new Set(rows.flatMap(row=>uniqueStrings(json(row.metadataJson)?.imageIds,MAX_IMAGES_PER_MESSAGE)))),images=new Map((await imageRows(db,organizationId,ids)).map(row=>[row.id,row]));return rows.map(row=>({role:row.role as "user"|"assistant",content:row.role==="user"?evidence(row.content,uniqueStrings(json(row.metadataJson)?.imageIds,MAX_IMAGES_PER_MESSAGE),images):row.content}));}
...
agenticWorkspaceRoutes.post("/workspace/images",requireScope("school:read"),async c=>{
 ...
 try{const scan=await scanSchoolImage(c.env.FINANCE_DB,c.env as any,p.organizationId,mime,bytes);await c.env.FINANCE_DB.prepare("UPDATE ae_image_attachments SET status='ready',ocr_text=?,vision_summary=?,error_text=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=?").bind(scan.ocrText,scan.visionSummary,id,p.organizationId).run();return c.json({data:{id,originalName:part.name,mimeType:mime,sizeBytes:part.size,status:"ready",ocrText:scan.ocrText,visionSummary:scan.visionSummary,model:scan.model,provider:scan.provider,previewPath:`/agentic-employees/vision/images/${id}`}},201);}catch(error){const message=error instanceof Error?error.message:String(error);await c.env.FINANCE_DB.prepare("UPDATE ae_image_attachments SET status='failed',error_text=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=?").bind(message.slice(0,1000),id,p.organizationId).run();throw error;}
});
...
agenticWorkspaceRoutes.get("/workspace/conversations/:id/messages",requireScope("school:read"),async c=>{
 ...
 const ids=Array.from(new Set(result.results.flatMap(row=>uniqueStrings(json(row.metadataJson)?.imageIds,MAX_IMAGES_PER_MESSAGE)))),images=new Map((await imageRows(c.env.FINANCE_DB,p.organizationId,ids)).map(row=>[row.id,row]));
 const messages=result.results.map(row=>({id:row.id,role:row.role,content:row.content,model:row.model||undefined,providerResponseId:row.providerResponseId||undefined,createdAt:row.createdAt,attachments:uniqueStrings(json(row.metadataJson)?.imageIds,MAX_IMAGES_PER_MESSAGE).map(id=>images.get(id)).filter(Boolean).map(image=>({kind:"image",id:image!.id,name:image!.originalName,mimeType:image!.mimeType,sizeBytes:image!.sizeBytes,status:image!.status,ocrText:image!.ocrText||"",visionSummary:image!.visionSummary||"",previewPath:`/agentic-employees/vision/images/${image!.id}`}))}));
...
agenticWorkspaceRoutes.post("/workspace/conversations/:id/messages",requireScope("school:read"),async c=>{
 const raw=await c.req.json().catch(()=>({})),parsed=z.object({content:z.string().trim().max(12000).optional().default(""),imageIds:z.array(z.string().min(1).max(100)).max(MAX_IMAGES_PER_MESSAGE).optional().default([])}).safeParse(raw);
 if(!parsed.success)throw new AppError(422,"VALIDATION_ERROR","Invalid AI message",parsed.error.flatten());
 const imageIds=uniqueStrings(parsed.data.imageIds,MAX_IMAGES_PER_MESSAGE),content=parsed.data.content.trim();if(!content&&!imageIds.length)throw new AppError(422,"MESSAGE_REQUIRED","Type a message or attach at least one image");
 const p=c.get("principal"),thread=await conversation(c.env.FINANCE_DB,p.organizationId,c.req.param("id"));if(!isAgentKey(thread.agentKey))throw new AppError(409,"AGENT_INVALID","Conversation agent is invalid");
