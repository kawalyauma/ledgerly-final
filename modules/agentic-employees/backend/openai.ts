...
import { executeTool, openAiTools } from "./tools-v14";
...
export async function runAgent(input: RunInput) {
  ...
  for (let round = 0; round < 5; round++) {
    ...
    if (!calls.length) return { text: textFrom(response) || "I could not produce a response.", model, providerResponseId: response.id || null, usage: response.usage || null, toolEvents };
    ...
    for (const call of calls) {
      ...
      await input.db.prepare(`INSERT INTO ae_tool_calls
        ...
        VALUES (?,?,?,?,?,?,?,'running')`).bind(logId,input.principal.organizationId,input.conversationId,input.agent.key,input.principal.userId,call.name!,JSON.stringify(args)).run();
      try {
        const result = await executeTool({ db: input.db, env: input.env, principal: input.principal, agent: input.agent, conversationId: input.conversationId, requestedTools: input.requestedTools }, call.name!, args);
    ...
    response = await callOpenAI(input.env, { model, previous_response_id: response.id, input: outputs, tools, tool_choice: "auto", parallel_tool_calls: false });
  }
  return { text: textFrom(response) || "The agent reached its tool-call limit. Please narrow the request.", model, providerResponseId: response.id || null, usage: response.usage || null, toolEvents };
}
