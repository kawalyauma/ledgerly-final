...
import { buildResponseLanguageBrief,buildResponseRealizationPrompt,cleanHumanResponse,composeFallbackHumanResponse,responseFingerprint,templateRisk } from "./response-intelligence/engine";
...
export async function runLightAgent(input:LightRunInput):Promise<LightResult>{
 ...
 const answer=await stage(input,"light_write_answer",()=>cheapText(runtime,`${languageBrief}\n\nAnswer using ONLY the verified Ledgerly results. Be concise but complete. Use a markdown table for exact comparisons when useful. Never invent missing values.\nUser request: ${prompt}\nVerified results: ${JSON.stringify(results).slice(0,22000)}`,1200));const text=cleanHumanResponse(answer.text||"I completed the Ledgerly lookup but could not format the final answer.");return{text,model:runtime.model,provider:runtime.provider,providerResponseId:answer.id,usage:answer.usage,toolEvents:events,routing:{...routing,responseFingerprint:responseFingerprint(text),templateRisk:templateRisk(text).risk}};
}
...
export async function runCompositeReport(input:LightRunInput,prompt:string){
 ...
 const data=sanitizeComposite(compiled.value,fallbackRows),summaryText=typeof data.summary==="string"?data.summary:(data.summary&&typeof data.summary==="object"?JSON.stringify(data.summary):""),reportSemantic={title:data.title,summary:summaryText,sections:data.calculations.length?[{title:"How the result was derived",analysis:data.calculations.join(". ")}]:[],findings:data.notes,limitations:data.missingData,rows:data.rows},recentRows=await input.db.prepare("SELECT content FROM ae_messages WHERE organization_id=? AND conversation_id=? AND role='assistant' ORDER BY created_at DESC,id DESC LIMIT 3").bind(input.principal.organizationId,input.conversationId).all<{content:string}>(),responseInput={purpose:"report" as const,request,seed:createId("rsp"),topic:"composite report",category:"report",detail:"standard" as const,audience:input.agent.title||input.agent.key,recentText:recentRows.results.map(row=>row.content).join("\n\n").slice(0,12000)};
 let humanResponse=composeFallbackHumanResponse(reportSemantic,responseInput);try{const realized=await stage(input,"composite_response_realize",()=>cheapText(runtime,buildResponseRealizationPrompt(responseInput,reportSemantic,sourcePayload),2400));if(realized.text.trim())humanResponse=cleanHumanResponse(realized.text);}catch{}
 return{mode:"composite-report",request,needsCriteria:false,questions:[],plan,data,humanResponse,responseMeta:{fingerprint:responseFingerprint(humanResponse),templateRisk:templateRisk(humanResponse).risk},sources:sourceInfo,toolEvents:events,model:runtime.model,provider:runtime.provider};
}
