...
export async function detectDerivedEmployeeEvents(env: Env) {
  const overdue = await env.FINANCE_DB.prepare(`SELECT
      p.id,
      p.organization_id AS organizationId,
      p.lesson_date AS lessonDate,
      p.status
    FROM school_lesson_plans p
    WHERE p.lesson_date<CURRENT_DATE
      AND p.status NOT IN ('delivered','cancelled')
      AND NOT EXISTS (
        SELECT 1
        FROM ae_event_inbox e
        WHERE e.organization_id=p.organization_id
          AND e.event_type='academics.lesson_plan_overdue'
          AND e.source_record_id=p.id
      )
    ORDER BY p.lesson_date ASC
    LIMIT 50
  `).all<any>();
  ...
  for (const row of overdue.results) {
    ...
    `).bind(
      createId("aev"),
      row.organizationId,
      "academics.lesson_plan_overdue",
      "academics",
      row.id,
      "lesson_plan",
      row.id,
      JSON.stringify({ lessonDate: row.lessonDate, status: row.status }),
    ).run();
  }
}
