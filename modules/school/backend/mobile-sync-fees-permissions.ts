...
async function allowed(c:MobileSyncAuthorizationContext,permission:string){
  const p=c.principal;
  ...
  return Boolean(await c.db.prepare(`SELECT 1 FROM school_user_roles ur JOIN school_role_permissions rp
...
export async function requireFeesRead(c:MobileSyncAuthorizationContext){if(!await allowed(c,"school.fees:read"))throw new AppError(403,"FORBIDDEN","Missing school permission: school.fees:read");}
export async function requireFeesWrite(c:MobileSyncAuthorizationContext){if(!await allowed(c,"school.fees:write"))throw new AppError(403,"FORBIDDEN","Missing school permission: school.fees:write");}
