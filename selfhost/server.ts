...
const scheduledTick=async()=>{if(scheduledRunning)return;scheduledRunning=true;try{for(const module of backendModules)if(module.key==="agentic-employees"&&module.scheduled)await module.scheduled(env as any);}catch(error){console.error(JSON.stringify({level:"error",message:"Agentic Employees scheduled tick failed",error:error instanceof Error?error.message:String(error)}));}finally{scheduledRunning=false;}};
const scheduledTimer=setInterval(()=>void scheduledTick(),60_000);scheduledTimer.unref();void scheduledTick();

const queueHandlers=new Map<string,(batch:any)=>Promise<void>>();
queueHandlers.set("finance-report-jobs",batch=>consumeReports(batch,env));
queueHandlers.set("finance-webhook-jobs",batch=>consumeWebhooks(batch,env));
for(const module of backendModules)for(const [name,fn] of Object.entries(module.queues||{}))queueHandlers.set(name,batch=>fn(batch,env));
let queuesRunning=false;
const queueTick=async()=>{if(queuesRunning)return;queuesRunning=true;try{for(const [name,fn] of queueHandlers){try{await drainPostgresQueue(db,name,fn,25);}catch(error){console.error(JSON.stringify({level:"error",message:"Self-host queue drain failed",queue:name,error:error instanceof Error?error.message:String(error)}));}}}finally{queuesRunning=false;}};
const queueTimer=setInterval(()=>void queueTick(),1000);queueTimer.unref();void queueTick();

const shutdown=async(signal:string)=>{console.info(JSON.stringify({level:"info",message:"Stopping Ledgerly Final self-host API",signal}));clearInterval(scheduledTimer);clearInterval(queueTimer);server.close();await db.close();process.exit(0);};
