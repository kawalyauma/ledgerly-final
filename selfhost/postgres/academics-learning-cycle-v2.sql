-- Academics learning cycle v2 PostgreSQL parity.
-- Apply after the core/school/academics schema exists.
-- These are lesson-level continuous/formative records; Exams remains separate.
...
CREATE INDEX IF NOT EXISTS acad_scheme_topics_scheme_idx ON acad_scheme_topics(organization_id,scheme_id,sequence_no,status);
...
CREATE INDEX IF NOT EXISTS acad_scheme_lessons_scheme_idx ON acad_scheme_lessons(organization_id,scheme_id,topic_id,status,sequence_no);
CREATE INDEX IF NOT EXISTS acad_scheme_lessons_date_idx ON acad_scheme_lessons(organization_id,planned_date,status);
...
CREATE INDEX IF NOT EXISTS acad_lesson_competencies_lesson_idx ON acad_lesson_competencies(organization_id,scheme_lesson_id,sequence_no);
...
CREATE INDEX IF NOT EXISTS acad_lesson_plan_links_lesson_idx ON acad_lesson_plan_links(organization_id,scheme_lesson_id);
...
CREATE INDEX IF NOT EXISTS acad_lesson_assessments_plan_idx ON acad_lesson_assessments(organization_id,lesson_plan_id,status);
...
CREATE INDEX IF NOT EXISTS acad_lesson_marks_assessment_idx ON acad_lesson_marks(organization_id,assessment_id,student_id);
