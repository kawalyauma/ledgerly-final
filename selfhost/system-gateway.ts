...
const modularPrefixes=["/api/v1/files"];
...
function pathOnly(path:string){return(path.split("?")[0]||path).replace(/\/$/,"");}
export function agentPathAllowed(agentKey:string,path:string){const p=pathOnly(cleanPath(path));if(blocked.some(x=>prefixMatch(p,x)))return false;return(profiles[agentKey]||[]).some(prefix=>prefixMatch(p,prefix));}
export function createAgentSystemGateway(app:any,getEnv:()=>any,secret:string,issuer:string,audience:string,coreApiUrl:string):AgentSystemGateway{
 ...
 return{
  ...
  async request(input){
   ...
   const modular=modularPrefixes.some(prefix=>prefixMatch(pathOnly(path),prefix));
   let response:Response;
   try{
    response=modular
      ?await app.fetch(new Request(`http://ledgerly.internal${path}`,{method,headers,body}),getEnv())
      :await fetch(`${baseUrl}${path}`,{method,headers,body,redirect:"manual"});
   }catch(error){return{ok:false,status:503,data:{error:{code:modular?"MODULAR_API_UNREACHABLE":"CORE_API_UNREACHABLE",message:modular?"Ledgerly modular API is unavailable":"Ledgerly core API is unavailable",details:{reason:error instanceof Error?error.message:String(error)}}}};}
