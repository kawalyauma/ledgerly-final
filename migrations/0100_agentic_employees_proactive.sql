CREATE TABLE IF NOT EXISTS ae_proactive_schedules (id TEXT PRIMARY KEY, organization_id TEXT NOT NULL, workflow_key TEXT NOT NULL, agent_key TEXT NOT NULL, actor_user_id TEXT NOT NULL, enabled INTEGER NOT NULL DEFAULT 1, cadence TEXT NOT NULL DEFAULT 'daily', run_hour INTEGER NOT NULL DEFAULT 7, run_minute INTEGER NOT NULL DEFAULT 0, weekday INTEGER, config_json TEXT NOT NULL DEFAULT '{}', last_run_at TEXT, next_run_at TEXT, updated_by TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE (organization_id, workflow_key));
...
CREATE INDEX IF NOT EXISTS idx_ae_proactive_runs_parent ON ae_proactive_runs(parent_run_id);
