import {recordKioskActivity} from "./kioskActivity";
...
export async function enqueue(event:AttendanceEvent){await OfflineStore.enqueue(event.clientEventId,JSON.stringify(event));await recordKioskActivity(event).catch(()=>{});return OfflineStore.count()}
